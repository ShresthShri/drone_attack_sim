...
from agents.ReLara_auxilary import AdaptiveDroneEnv  # Import your environment

def evaluate_drone_with_trajectory(model_path="adaptive_drone_navigation", num_episodes=5):
    ...
    for episode in range(num_episodes):
        ...
        print(f"Env reward: {info.get('env_reward', 0):.2f}")
        print(f"Aux reward: {info.get('aux_reward', 0):.2f}")
        print(f"Exploration phase: {info.get('exploration_phase', 'N/A')}")
        ...
        result = {
            ...
            'aux_reward': info.get('aux_reward', 0),
            'exploration_phase': info.get('exploration_phase', False)
        }
...
def plot_summary(all_results):
    ...
    ax3.axhline(y=0.5, color='r', linestyle='--', label='Goal Threshold (0.5)')
    ...
    # Additional ReLara-specific statistics
    if all_results and 'env_reward' in all_results[0]:
        env_rewards = [r['env_reward'] for r in all_results]
        aux_rewards = [r['aux_reward'] for r in all_results]
        exploration_episodes = sum(1 for r in all_results if r['exploration_phase'])
        
        print(f"\nReLara Statistics:")
        print(f"Average Environmental Reward: {np.mean(env_rewards):.1f}")
        print(f"Average Auxiliary Reward: {np.mean(aux_rewards):.1f}")
        print(f"Episodes in Exploration Phase: {exploration_episodes}/{len(all_results)}")
        print(f"Reward Agent Contribution: {np.mean(aux_rewards) / np.mean(rewards) * 100:.1f}%")
